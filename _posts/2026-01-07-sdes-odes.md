...
[^6]: This is technically somewhat imprecise.
The derivative of $\psi_t(x)$ is _not_ $u_t(x)$ -- it's $u_t(\psi_t(x))$.
This is why the $u$ functions given in the paper for specific probability path look more complex than they might need to be -- they are given as $u_t(x)$, but we generally use the form $u_t(\psi_t(x))$.
